expMST <- function(gm, cutoff=0.05, min.size=3){
  g <- igraph::graph.adjacency(-log1p(-gm), weighted=T)
  mst <- igraph::mst(g)
...
minPath <- function(s, t, gm, clusters,
  ...
  return(list(pvals=pvals, clusters=nodeClusters-1,
              names=nodeNames))
}

.plotPath <- function(sp_vert, clusters, nodeNames, nodeClusters){
  gdf <- data.frame(from=seq_len(length(sp_vert)-1), to=c(2:length(sp_vert)))
  ...
  plot(gdf, layout=igraph::layout_as_tree, vertex.label=nodeNames,
...
distBoot <- function(x, partial=FALSE, squared=TRUE, output="orig"){
  ...
  gmShuf <- gelMatrix(xShuf, squared=squared, partial=partial, output=output)
  D <- -log1p(-gmShuf)
  diag(D) <- 0
...
pBoot <- function(d, z){
  delta <- z - d
  p <- pgamma(delta[delta>0], 2, lower.tail=F, log.p=T)
  log.p <- sum(p)
  return (1-exp(log.p))
}
